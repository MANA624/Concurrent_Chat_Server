...
class ServerTest implements Runnable{
    private static final int NUM_TESTERS = 16;
    private String name;
    ...
    public static void main(String[] args){
        ...
        String path = System.getProperty("user.dir") + "/src/TestCode/";
        ...
        System.out.print("What kind of message are you sending? ('s'mall, 'm'edium, or 'l'arge): ");
        userInput = reader.nextLine();
        if(userInput.equalsIgnoreCase("s")){
            messageToSend = "Hello world! How are you?";
        }
        else if(userInput.equalsIgnoreCase("m")){
            path += "Episode5.txt";
            messageToSend = readFile(path);
        }
        else if(userInput.equalsIgnoreCase("l")){
            path += "AllEpisodes.txt";
            messageToSend = readFile(path);
        }
        else{
            System.out.println("Not a valid input");
